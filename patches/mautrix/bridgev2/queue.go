...
func (br *Bridge) QueueMatrixEvent(ctx context.Context, evt *event.Event) EventHandlingResult {
	...
	// Check if there's a pending command state (e.g., waiting for login input)
	// and route to command handler even if message doesn't start with command prefix.
	if sender != nil && sender.HasCommandState() && evt.Type == event.EventMessage {
		msg := evt.Content.AsMessage()
		msg.RemoveReplyFallback()
		msg.RemovePerMessageProfileFallback()
		if !sender.Permissions.Commands {
